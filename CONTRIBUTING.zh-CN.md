...
- 未包含包 manifest 时不写 `manifests`。写入时，每个 route 都映射到
...
Manifest 文件使用 `kind: manifest`，并且 manifest `name` 必须匹配根 route：
...
- 只有对应 manifest 文件存在并通过校验时，才在根 `fluoh.yaml` 加入
  `manifests` route。
- 同一个 package name 只能出现在一个 manifest route 中。
...
当 route 已存在、并指向已有 release tag 的包仓库时，使用 `fluoh source sync`
刷新 release records。Route metadata、advisory 文案和 frozen maintenance 说明可直接
编辑 manifest 文件。
`fluoh source sync` 不会自动发现新的官方 route；首次接入一个包时，PR 必须同时加入
根 route 和对应的 `manifests/<route>/fluoh.yaml`。

首次接入适配包时，通过本仓库 PR 完成：

1. Fork 或 clone `FlutterOH/source`。
2. 选择稳定的 route 名称，通常使用包名或包组名。
3. 在根 `fluoh.yaml` 添加 route：
